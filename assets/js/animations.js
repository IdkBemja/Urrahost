...
contactTriggers.forEach(button => {
    button.addEventListener('click', function(e) {
        ...
        switch(service) {
            case 'web':
                message = 'Hola! Me interesa el plan de Alojamiento Web. ¿Podrían darme más información?';
                break;
            case 'db':
                message = 'Hola! Me interesa el plan de Base de Datos. ¿Podrían darme más información?';
                break;
            case 'development':
                message = 'Hola! Me interesa el servicio de Desarrollo Web. ¿Podrían enviarme una cotización?';
                break;
            default:
            ...
            try {
                ...
                if (navigator.clipboard) {
                    navigator.clipboard.writeText(message).then(() => {
                        alert('No se pudo abrir WhatsApp. El mensaje se ha copiado al portapapeles.');
                    });
                }
            }
        }, 150);
...
function animateShapes() {
    shapes.forEach((shape, index) => {
        ...
        shape.style.transform = `translate(${Math.sin(scroll) * 20}px, ${Math.cos(scroll) * 20}px) rotate(${scroll * 10}deg)`;
    });
...
// ==================== Console Message ====================
console.log('%c🚀 Urra Hosting ', 'background: linear-gradient(135deg, #F97316 0%, #EA580C 100%); color: white; padding: 10px 20px; font-size: 20px; font-weight: bold; border-radius: 5px;');
console.log('%c✨ Diseñado con tecnología moderna y amor', 'color: #F97316; font-size: 14px;');
console.log('%c💼 ¿Interesado en nuestros servicios? Contáctanos en https://wa.me/56937641844', 'color: #666; font-size: 12px;');
